...
public class EventsDAOImpl implements EventDAO {
	...
	public void insertMarkedEvents(List<Event> markedEvents) {
		...
		insertEvents(writeDB, markedEvents); // még nem törölte mikor már
												// insertált (mikor még külön
												// volt)
		writeDB.close();
	}
	...
	Object getEvents(SQLiteDatabase db) {
		...
		Cursor cEvent = db.query("Event", new String[] { "*" }, null, null, null, null, null);
		if (cEvent.moveToFirst()) {
			while (!cEvent.isAfterLast()) {
				...
				Cursor cCategories = db.query("Categories", new String[] { "name" }, "eventID=?", new String[] { "" + id }, null, null, null);
				...
				Cursor cProviders = db.query("Providers", new String[] { "name" }, "eventID=?", new String[] { "" + id }, null, null, null);
				...
				Cursor cLocations = db.query("Locations", new String[] { "name" }, "eventID=?", new String[] { "" + id }, null, null, null);
				...
				Cursor cRanks = db.query("RankResult", new String[] { "rankID", "title", "count", "rankValue" }, "eventID=?",
						new String[] { "" + id }, null, null, null);
				if (cRanks.moveToFirst()) {
					while (!cRanks.isAfterLast()) {
						rankResults.add(new EventRankResult(cRanks.getLong(0), cRanks.getString(1), cRanks.getLong(2), cRanks.getDouble(3)));
						cRanks.move(1);
					}
				}
				cRanks.close();
				Event event = new Event(markerType, markerRemark, title, description, startDate, endDate, categories, providers, locations,
						rankResults, id, provided, rankable, editable);
				events.add(event);
				cEvent.move(1);
			}
		}
	...
	void insertEvents(SQLiteDatabase db, List<Event> markedEvents) {
		...
		for (Event e : markedEvents) {
			db.execSQL("INSERT INTO Event VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);", new Object[] { e.getEventId(), e.getTitle(), e.getDescription(),
					e.getEventStart().getTimeInMillis(), e.getEventEnd().getTimeInMillis(), e.getMarkerType(), e.getMarkerRemark(),
					e.isProvided() == true ? 1 : 0, e.isRankable() == true ? 1 : 0, e.isEditable() == true ? 1 : 0 });
			...
			for (EventRankResult r : e.getRankResult()) {
				db.execSQL("INSERT INTO RankResult VALUES (?, ?, ?, ?, ?);", new Object[] { r.getRankId(), e.getEventId(), r.getTitle(),
						r.getCount(), r.getRankValue() });
			}
		}
	}
